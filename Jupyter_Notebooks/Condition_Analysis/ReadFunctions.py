...
def read_fitness(fileLoc):
    ...
    df_fitness = pd.read_csv(fileLoc, skiprows=18, delim_whitespace=True, header=0, names=fitness_best_names)
    return df_fitness
...
def read_bp(fileLoc):
    ...
    df_bp = pd.read_csv(fileLoc, skiprows=17, delim_whitespace=True, header=0, names=bp_best_names)
    return df_bp
...
def read_genes(fileLoc):
	...
	df_genes = pd.read_csv(fileLoc, skiprows=14, delim_whitespace=True,header=0, names=genes_best_names)
	return df_genes
    
def read_robustness(fileLoc):
    robustness_names = []
    df_robustness = pd.read_csv(fileLoc, skiprows= x, delim_whitespace=True, header=0, names=robustness_names)
