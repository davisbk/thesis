...
This shows that both mutational robustness and evolvability are low, which is to be expected after 10.5 million generations in a static environment. 
% MUTATION UP
...
\begin{figure}[H]
	...
	\caption[Selection up percent non-coding]{$k_+$ percent non-coding, generations 1-500,000, all seeds.}
...
\end{figure}
The slight increase in genome size is probably best explained by the gaining of non-coding bases, as shown in Figure~\ref{fig:selection_down_perc_non-coding}.
\begin{figure}[H]
	\centering
...
\end{table}
Since the $k_-$ condition's p-value was $\geq0.5$, it must be rejected.
