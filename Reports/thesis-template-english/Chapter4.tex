...
Figure~\ref{fig:control_avg_size_of_functional_genes_b1-e500000} shows the change in the average size of functional genes observed in the control condition for all seeds for all 500,000 generations. Gene density 
...
Figure~\ref{fig:control_gene_density} shows gene density (number of genes per total number of base pairs). This is a convenient measure for taking into account both the number of genes and the genome size. 
...
At first glance, this increase in genome size seems counter to the prediction based on Liard et al.~\cite{Liard.2018} as discussed in Section~\ref{related_work:reductive_evolution}. Based on their research, in the $\mu_+$ condition, an increased mutation rate is expected to boost mutational robustness by disrupting existing genes and introducing many non-coding bases, which in turn allows purifying selection to overcome the proposed complexity ratchet, finally reducing the genome. This does seem to be what happened in these experiments, as well, as the final genome size was below the control condition and seemingly on its way down. 
...
The robustness scores for the $\mu_+$ condition seen in Table~\ref{table:robustness_means_and_std_dev} (pg.~\pageref{table:robustness_means_and_std_dev}) indicate that increasing the mutation rate did not produce this effect on robustness, as mutational robustness actually went down from the control condition. Contrary to Liard et al., despite the increased mutation rate in this condition, the number of non-coding bases went down (see Figure~\ref{fig:mut_up_percent_non-coding} below) and Table~\ref{table:non-coding_DNA_mean_and_standard_deviation}, pg.~\pageref{table:non-coding_DNA_mean_and_standard_deviation}), the gene density went up (see Figure~\ref{fig:mut_up_gene_density}), and the number of genes overall nevertheless went down (see Table~\ref{table:number_of_genes_mean_std_dev}, pg.~\pageref{table:number_of_genes_mean_std_dev}).
...
\end{table}
\subsubsection{Number of Non-Functional Genes}\label{results:number_non-functional_genes}
...
%TODO Fix this next paragraph. Does it really follow from the previous one? What's the connection?
