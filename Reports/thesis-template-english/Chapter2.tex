...
Evolvability is usually defined as the ability of a system (in this case an organism) to evolve. In other words, a system has evolvability ``if mutations in it can produce heritable phenotypic variation''\cite{doi:10.1098/rspb.2007.1137}. However, of critical importance is that this is not simply having a large amount of genetic diversity, but rather \textit{adaptive} diversity which provides some benefit. There are also competing definitions of evolvability, as some define it as "%TODO Finish this sentence

%TODO How are evolvability and selection linked?
There is a link between evolvability and selection, in the sense that a trait which confers some fitness advantage may be selected for and a genome with high evolvability may contain this trait. %TODO Finish this section
...
Batut et al.~\cite{Batut.2014} investigated the idea of ``Muller's ratchet'' and whether it might explain the genome reduction seen in \textit{Prochlorococcus}. In Muller's ratchet, organisms with smaller effective population sizes, such as endosymbiotic bacteria like \textit{Buchnera aphidicola}, may lose genes because deleterious mutations accumulate in a population due to the smaller population size not allowing for enough variability. In the absence of recombination, these deleterious mutations become fixed in a population. The idea was originally put forth by H.J. Muller~\cite{MullersRatchet}. Batut et al. found that generally, the 
