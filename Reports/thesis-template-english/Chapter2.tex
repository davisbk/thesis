...
\section{Changeable Factors}\label{aevol_post-treatments}

Once the experiments have completed, Aevol by default produces several statistics files which include information about genome size, the percentage of coding DNA, number of genes, average metabolic error, and many other factors.  It further includes a number of post-treatments that allow one to analyze specific individuals or the population at large, including tools for determining robustness, evolvability, coalescence, and the lineages. 
...
As described in Section \ref{subsec:aevol_selection} and illustrated in Figure \ref{fig:aevol_fitness01}, fitness in Aevol is measured as the gap $g$ between the environment and the phenotype, i.e.  $g = \int_{a}^{b} f_E(x) - f_P(x) dx$. Aevol provides many statistics about fitness, including the population size (for cases when it is not fixed), overall fitness, genome size, metabolic error	  

\subsection{Structure}
Another strength of Aevol is its ability to analyze changes in the structure of DNA. As mentioned above, Aevol produces statistics about individuals and the population at large, including the percentage of coding vs. non-coding DNA, the number of genes,  In the following subsections we will examine 
