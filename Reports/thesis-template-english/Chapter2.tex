...
\subsection{Advantages of Aevol}
...
Liard et al. showed in \cite{doi:10.1162} that selection for fitness is not necessarily enough to overcome the tendency of organisms to become more and more complex. Selecting for \textit{robustness}, however, (rather than fitness) can overcome the tendency of organisms to become more and more complex, what they describe as a ``complexity ratchet''. They describe the problem:
\begin{quote}
	``Since gene deletion is obviously deleterious, the only available evolutionary path for already complex organisms is a headlong rush toward increasing complexity by acquiring new genes. Hence the ratchet clicks, further widening the fitness valley that separates the current genome from a simple one, soon making it so wide it is very unlikely to be crossed."
\end{quote} 
They found that one solution to this problem was to select for robustness
...
%TODO Mention "The Complexity Ratchet: Stronger than Selection, Weaker Than Robustness"
